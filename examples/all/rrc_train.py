...
#dataset_name = "SKKU" # SKKU dataset with scene frame coords.
dataset_name = "KITTI" # KITTI dataset with scene frame coords.
...
def AddExtraLayers(net):
    ...
    ##  For ResNet-18
    net["conv4_3r"] = L.Convolution(net["res4b"], num_output=256, kernel_size=3, pad=1, stride=1, **kwargs)
    net["conv4_3r_relu"] = L.ReLU(net["conv4_3r"], in_place=True)
    net["fc7r"] = L.Convolution(net["res5b"], num_output=256, kernel_size=3, pad=1, stride=1, **kwargs)
    net["fc7r_relu"] = L.ReLU(net["fc7r"], in_place=True)
    net["conv6_1"] = L.Convolution(net["res5b_relu"], num_output=256, kernel_size=1, pad=0, stride=1, **kwargs)
    net["conv6_1_relu"] = L.ReLU(net["conv6_1"], in_place=True)

    net["conv6_2"] = L.Convolution(net["conv6_1"], num_output=256, kernel_size=3, pad=1, stride=2, **kwargs)
    net["conv6_2_relu"] = L.ReLU(net["conv6_2"], in_place=True)

    net["conv7_1"] = L.Convolution(net["conv6_2"], num_output=128, kernel_size=1, pad=0, stride=1, **kwargs)
    net["conv7_1_relu"] = L.ReLU(net["conv7_1"], in_place=True)

    net["conv7_2"] = L.Convolution(net["conv7_1"], num_output=256, kernel_size=3, pad=1, stride=2, **kwargs)
    net["conv7_2_relu"] = L.ReLU(net["conv7_2"], in_place=True)

    net["conv8_1"] = L.Convolution(net["conv7_2"], num_output=128, kernel_size=1, pad=0, stride=1, **kwargs)
    net["conv8_1_relu"] = L.ReLU(net["conv8_1"], in_place=True)

    net["conv8_2"] = L.Convolution(net["conv8_1"], num_output=256, kernel_size=3, pad=1, stride=2, **kwargs)
    net["conv8_2_relu"] = L.ReLU(net["conv8_2"], in_place=True)
...
if dataset_name == "KITTI":
    resize_width = 2560
    resize_height = 768
elif dataset_name == "SKKU_laneCar":
    resize_width = 1024
    resize_height = 500
else: # SKKU_car or other dataset
    resize_width = 1280
    resize_height = 768
