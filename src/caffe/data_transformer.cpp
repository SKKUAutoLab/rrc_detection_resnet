...
void DataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
  ...
  if(param_.hsv() && Rand(2)){
     ...
     cv::cvtColor(cv_img,cv_hsv_img,cv::COLOR_BGR2HSV);
     ...
     cv::cvtColor(cv_hsv_img,cv_img,cv::COLOR_HSV2BGR);
  }
...
//     int name_idx = rand();
//     std::string filename = "/home/ubuntu/xiaohao/temp/";
//     std::stringstream aaa;
//     aaa << name_idx;
//
//     filename = filename + aaa.str() + ".jpg";
//     cv::imwrite(filename.c_str(),cv_img);
  const Dtype scale = param_.scale();
  *do_mirror = param_.mirror() && Rand(2);
  const bool has_mean_file = param_.has_mean_file();
  const bool has_mean_values = mean_values_.size() > 0;
